...
export default function Slide1({x, p}:Props) {
  return <Wrapper overflow={'visible'}>
    ...
    <PauloFreire p={p} x={x} v={0.5}>
      ...
      <Css p={p} x={x} v={0.6} src="/assets/img/stack/css.svg" alt="css" />
      <Js p={p} x={x} v={1} src="/assets/img/stack/js.svg" alt="js" />
      <Ts p={p} x={x} v={2} src="/assets/img/stack/ts.svg" alt="ts" />
      <React p={p} x={x} v={3} src="/assets/img/stack/react.svg" alt="react" />
      <Github p={p} x={x} v={2} src="/assets/img/stack/github.svg" alt="GitHub" />
      <Next p={p} x={x} v={0.5} src="/assets/img/stack/next.svg" alt="Next" />
      <Mysql p={p} x={x} v={1} src="/assets/img/stack/mysql.svg" alt="Mysql" />
      <Php p={p} x={x} v={0.5} src="/assets/img/stack/php.svg" alt="Php" />
      <Netlify p={p} x={x} v={0.5} src="/assets/img/stack/netlify.svg" alt="Netlify" />
    </PauloFreire>
  </Wrapper>
}
...
const Img = styled.img<ParallaxProps>`
  ...
  top:0;
  left:0;
  transform: translateY(${({x, v})=> `${x*v}px`}) translateX(${({x, v})=> `${x*v*-2}px`});
`
